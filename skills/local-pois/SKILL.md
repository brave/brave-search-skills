---
name: local-pois
description: USE FOR getting local business/POI details via Search API. Requires POI IDs obtained from web-search (with result_filter=locations). Returns full business information including ratings, hours, contact info. Max 20 IDs. Requires BRAVE_SEARCH_API_KEY.
---
...
| `rating.is_tripadvisor` | bool? | Whether rating is from Tripadvisor |
...
| `reviews.results` | object[]? | Tripadvisor reviews |
| `reviews.viewMoreUrl` | string? | Link to more reviews |
...
| `pictures.viewMoreUrl` | string? | Link to more photos |
...
- **ID validity**: POI IDs expire after ~8 hours — refetch if stale
