...
description: USE FOR getting AI-generated POI text descriptions via Search API. Requires POI IDs obtained from web-search (with result_filter=locations). Returns markdown descriptions grounded in web search context. Max 20 IDs per request. Requires BRAVE_SEARCH_API_KEY.
---
...
- **No contact/operational data**: Descriptions never include addresses, phone numbers, hours, ratings, or URLs — use `local-pois` for that
- **AI-generated**: Descriptions are AI-generated based on web search context, not sourced from business profiles
- **Timeout**: Recommended 30s (LLM generation can be slow on first call)
